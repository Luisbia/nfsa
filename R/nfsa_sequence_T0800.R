...
#'
#' @description This function takes T0800 data or revisions, formats the output, and exports it to an Excel file based on a
...
#'   nfsa_sequence_T0800(country_sel = "BE", type = "values")
#'
#'   # Example usage with multiple countries and custom time period
#'   nfsa_sequence_T0800(country_sel = c("BE", "NL"), time_min = 2021, type = "revisions")
#'
#' }
#'
#' @export
nfsa_sequence_T0800 <- function(country_sel,
                                time_min = 2020,
                                type = "new",
...
if (type == "values"){
  ...
  if(length(country_sel) == 1) {
    saveWorkbook(wb, ,file =paste0(output_sel,"/T0800_",country_sel,"_",as.character(format(Sys.time(), "%Y%m%d_%H%M%S")),"_seq_accounts.xlsx"))

    cli::cli_alert_success(paste0("File created in ",output_sel,"/T0800_",country_sel,"_",as.character(format(Sys.time(), "%Y%m%d_%H%M%S")),"_seq_accounts.xlsx"))
  }
