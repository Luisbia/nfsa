...
#' @param template A character string specifying the file path to the Excel template file. Default: `here("assets","seq_accounts_A.xlsx")`.
...
#'
#' @export
nfsa_sequence_T0800 <- function(country,
                                time_min = 2020,
                                type = "values",
                                template = here("assets","seq_accounts_A.xlsx"),
...
nfsa_prev_data <- nfsa::nfsa_get_data(country = country, table = "T0800", type = "prev") |>
  separate_wider_delim(cols = id,
                       delim = ".",
                       names = c("ref_sector", "sto", "accounting_entry")) |>
  filter(ref_sector %in% c("S1", "S1N", "S11", "S12", "S13","S1M", "S2"),
