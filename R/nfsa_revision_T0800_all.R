...
#' nfsa_revision_T0800_all(country_sel = "IT")
#'
#' # Example usage: Identify and extract revisions for country code "BE"
#' # from Parquet files located in a custom directory.
#' nfsa_revision_T0800_all(country_sel = "BE", path_sel = "path/to/my/data")
#' }
#'
#' @export
nfsa_revision_T0800_all <- function(country_sel,
  ...
  revisions <- list.files(path = path_sel,
                          pattern = paste0("^NASEC_T0800_A_", country_sel, "_.*\\.parquet$"),
                          full.names = TRUE,
                          recursive = TRUE) |>
    open_dataset() |>
    collect() %>%
