...
#'
#' @export
nfsa_a_plots_T_V <- function(country_sel,
...
tmp_t <- nfsa::nfsa_get_data(country_sel = country_sel,
                             table_sel = "T0800",
                             type = "new") |>
...
tmp_v<- nfsa::nfsa_get_data(country_sel = country_sel,
                            table_sel = "T0800",
                            type = "prev") |>
  filter(time_period >= time_min) |>
  separate_wider_delim(id,delim = ".", names = c("ref_sector","sto","accounting_entry")) |>
...
ggsave(
  filename = paste0(output_sel,"/", country_sel,"_a_T_V.pdf"),
  plot = marrangeGrob(charts, nrow=1, ncol=1),
  width = 15, height = 9
)
cli::cli_alert_success(paste0("Charts created in ",output_sel,"/", country_sel,"_a_T_V.pdf"))
}
