...
nfsa_theme_new <- function(base_size = 12) {
library(ggplot2)
  ...
  theme_minimal(
    ...
    panel.grid.major.y = ggplot2::element_line(color = "#D4D5D7", linetype = "dashed", linewidth = 1),
...
tmp <- nfsa_get_data(country = "ES") |>
  filter(id == "S1.B1GQ.B") |>
  mutate(time_period = lubridate::yq(time_period))

ggplot(tmp,aes(time_period,obs_value))+
  geom_line()+
  nfsa_theme_new(base_size = 18)
