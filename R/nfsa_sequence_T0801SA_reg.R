#' @title Show data as sequence of accounts in ESA 2010 TP Regulation
#'
#' @description This function takes T0801 data, formats the output, and exports it to an Excel file based on a
...
#' @param template A character string specifying the file path to the Excel template file. Default: `here("assets","seq_accounts_T0801_reg.xlsx")`.
...
#'     \item Formats the data and creates an Excel file using the specified template via `openxlsx`.
...
#'
#' @export
nfsa_sequence_T0801SA_reg <- function(country,
                                time_min = "2020-Q1",
                                type = "values",
...
nfsa_new_data <- nfsa::nfsa_get_data(country = country, table = "T0801SA", type = "new") |>
  ...
  if(length(country) == 1) {
    saveWorkbook(wb, ,file =paste0(output_sel,"/T0801SA_",country,"_",as.character(format(Sys.time(), "%Y%m%d_%H%M%S")),"_seq_accounts_reg.xlsx"))

    cli::cli_alert_success(paste0("File created in ",output_sel,"/T0801SA_",country,"_",as.character(format(Sys.time(), "%Y%m%d_%H%M%S")),"_seq_accounts_reg.xlsx"))
  }

  if(length(country) > 1) {
    saveWorkbook(wb, ,file = paste0(output_sel,"/T0801SA_",as.character(format(Sys.time(), "%Y%m%d_%H%M%S")),"_seq_accounts_reg.xlsx"))

    cli::cli_alert_success(paste0("File created in ",output_sel,"/T0801SA_",as.character(format(Sys.time(), "%Y%m%d_%H%M%S")),"_seq_accounts_reg.xlsx"))
  }
