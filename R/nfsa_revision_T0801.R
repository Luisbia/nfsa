...
nfsa_revision_T0801 <- function(country,
  ...
  if (type == "version"){
    prev_db <-  nfsa_new_files <- list.files(path = "M:/nas/Rprod/data/q/new/nsa/",
                                             recursive = FALSE,
                                             full.names = TRUE) |>
      as_tibble() |>
  ...
  if(length(unique(new_prev_db$ref_area)) == 1) {
    openxlsx::write.xlsx(new_prev_db,
                         file = paste0(output_sel,"/revisions_T0801_",unique(new_prev_db$ref_area),"_",as.character(format(Sys.time(), "%Y%m%d_%H%M%S")),".xlsx"),
                         overwrite = TRUE,
                         asTable = TRUE)
