...
#'   nfsa_sequence_T0801_reg(country = "BE")
#'
#'   # Example usage with multiple countries and custom time period
#'   nfsa_sequence_T0801_reg(country = c("BE", "NL"), time_min = 2021)
#'
#' }
#'
#' @export
...
nfsa_new_data <- nfsa::nfsa_get_data(country = country, table = "T0801", type = "new") |>
  ...
  if(length(country) == 1) {
    saveWorkbook(wb, ,file =paste0(output_sel,"/T0801_",country,"_",as.character(format(Sys.time(), "%Y%m%d_%H%M%S")),"_seq_accounts_reg.xlsx"))

    cli::cli_alert_success(paste0("File created in ",output_sel,"/T0801_",country,"_",as.character(format(Sys.time(), "%Y%m%d_%H%M%S")),"_seq_accounts_reg.xlsx"))
  }

  if(length(country) > 1) {
    saveWorkbook(wb, ,file = paste0(output_sel,"/T0801_",as.character(format(Sys.time(), "%Y%m%d_%H%M%S")),"_seq_accounts_reg.xlsx"))

    cli::cli_alert_success(paste0("File created in ",output_sel,"/T0801_",as.character(format(Sys.time(), "%Y%m%d_%H%M%S")),"_seq_accounts_reg.xlsx"))
  }
