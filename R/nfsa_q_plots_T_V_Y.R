...
#' @examples
#' \dontrun{
#' nfsa_q_plots_T_V_Y(country_sel = "AT", output_sel = here("my_output"))
#' }
#'
#' @export
nfsa_q_plots_T_V_Y <- function(country_sel,
...
tmp_t <- nfsa::nfsa_get_data(country_sel = country_sel, table_sel = "T0801SA", type = "new") |>
...
if(nrow(tmp_t) == 0) stop(paste0("No seasonaly adjusted file for ", country_sel))


tmp_v<- nfsa::nfsa_get_data(country_sel = country_sel, table_sel = "T0801SA", type = "prev") |>
  filter(obs_value != "NaN") |>
  filter(time_period >= time_min) |>
  separate_wider_delim(id,delim = ".", names = c("ref_sector","sto","accounting_entry")) |>
  mutate(source = "V",
...
if(nrow(tmp_v) == 0) stop(paste0("No seasonaly adjusted file for ", country_sel))
...
ggsave(
  filename = paste0(output_sel,"/", country_sel,"_q_T_V_Y.pdf"),
  plot = marrangeGrob(charts, nrow=1, ncol=1),
  width = 15, height = 9
)
cli::cli_alert_success(paste0("Charts created in ",output_sel,"/", country_sel,"_q_T_V_Y.pdf"))
}
