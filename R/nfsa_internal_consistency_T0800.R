...
nfsa_internal_consistency_T0800 <- function(dataset,
  ...
  ur10 <- data |>
   ...
   nfsa_unite_id() |>
    filter(id %in% c("S1.D442.D", "S2.D442.C", "S1.D614.C", "S2.D614.D")) |>
  ...
  sit03 <- data |>
    ...
    select(ref_area, ref_sector, time_period, P51G.D, P52.D,P53.D, P5.D) |>
    rowwise() |>
    mutate(`P51G.D + P52.D + P53.D` = sum(P51G.D,P52.D,P53.D, na.rm = TRUE),
  ...
  sit05 <- data |>
    ...
    select(ref_area, ref_sector, accounting_entry,time_period, D11, D12, D1) |>
    rowwise() |>
    mutate(`D11 + D12` = sum(D11,D12, na.rm = TRUE),
  ...
  sit22 <- data |>
    ...
    select(ref_area, ref_sector, accounting_entry,time_period, D611, D612, D613, D614, D61SC, D61) |>
    rowwise() |>
    mutate(`sum_D61` = sum(c(D611, D612, D613, D614, -D61SC), na.rm = TRUE),
  ...
  sit24 <- data |>
    ...
    select(ref_area, ref_sector, accounting_entry,time_period, D71, D72,  D74, D75, D76, D7) |>
    rowwise() |>
  ...
  sit25 <- data |>
    ...
    select(ref_area, ref_sector, accounting_entry,time_period, D71, D75, D7) |>
    rowwise() |>
    mutate(`D71 + D75` = sum(c(D71, D75), na.rm = TRUE),
  ...
  sit26 <- data |>
    ...
    select(ref_area, ref_sector, accounting_entry,time_period, D71, D72, D75, D7) |>
    rowwise() |>
    mutate(`D71 + D72 + D75` = sum(c(D71,D72, D75), na.rm = TRUE),
  ...
  sit27 <- data |>
    ...
    select(ref_area, ref_sector, accounting_entry,time_period, D71, D72, D74, D75, D7) |>
    rowwise() |>
    mutate(`D71 + D72 + D74 + D75` = sum(c(D71,D72,D74, D75), na.rm = TRUE),
  ...
  sit29 <- data |>
    ...
    select(ref_area, ref_sector, accounting_entry,time_period, D91, D92, D99, D9) |>
    rowwise() |>
    mutate(`D91 + D92 + D99` = sum(c(D91,D92,D99), na.rm = TRUE),
  ...
  sit30 <- data |>
    ...
    select(ref_area, ref_sector, accounting_entry,time_period, D91,  D99, D9) |>
    rowwise() |>
    mutate(`D91 + D99` = sum(c(D91,D99), na.rm = TRUE),
  ...
  sit30b <- data |>
    ...
    select(ref_area, ref_sector, accounting_entry,time_period, D92,  D99, D9) |>
    rowwise() |>
  ...
  sit33 <- data |>
    ...
    select(ref_area, ref_sector, accounting_entry,time_period, D211, D212, D214, D21) |>
    rowwise() |>
    mutate(`D211 + D212 + D214` = sum(c(D211, D212, D214), na.rm = TRUE),
           check = round(D21 - `D211 + D212 + D214`, rounding)) |>
    filter(abs(check) > threshold) |>
    filter(if_all(c(D211, D212, D214), ~ !is.na(.x )))
  ...
  sit37 <- data |>
    ...
    select(ref_area, ref_sector, accounting_entry,time_period, D72, D75, D7) |>
    rowwise() |>
    mutate(`D72 + D75` = sum(c(D72, D75), na.rm = TRUE),
  ...
  sit39 <- data |>
    ...
    select(ref_area, ref_sector, accounting_entry,time_period, P11, P12,P13, P1) |>
    rowwise() |>
    mutate(`P11 + P12 + P13` = sum(c(P11, P12, P13), na.rm = TRUE),
  ...
  sit40 <- data |>
    ...
    select(ref_area, ref_sector, accounting_entry,time_period, P11, P12, P1) |>
    rowwise() |>
    mutate(`P11 + P12` = sum(c(P11, P12), na.rm = TRUE),
  ...
  BI06 <- data |>
    ...
    pivot_wider(names_from = sto,
                values_from = obs_value) |>
    select(ref_area, ref_sector, time_period, B1N.B, B1G.B) |>
    rowwise() |>
    mutate(
      `check` = B1G.B - B1N.B
    ) |>
  ...
  BI11 <- data |>
    ...
    pivot_wider(names_from = sto,
                values_from = obs_value) |>
    select(ref_area, ref_sector, time_period, B2G.B, B3G.B,B2A3G.B) |>
    rowwise() |>
    mutate(
  ...
  #   pivot_wider(names_from = sto,
  #               values_from = obs_value) |>
  #   select(ref_area, ref_sector, time_period, B2G.B, B2A3G.B) |>
  #   mutate(check = round(B2A3G.B - B2G.B, rounding)
  #   ) |>
  #   filter(abs(check) > threshold)
  ...
  BI27 <- data |>
    ...
    pivot_wider(names_from = sto,
                values_from = obs_value) |>
    select(ref_area, ref_sector, time_period, B6G.B,D63.C,B7G.B) |>
    rowwise() |>
    mutate(
