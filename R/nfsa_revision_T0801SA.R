#' Detects and reports revisions in T0801sa data between two versions of the database.
#'
...
#' nfsa_revision_T0801SA(country = c("ES", "IT"))
#'
#' # Example usage with custom thresholds:
#' nfsa_revision_T0801SA(country = c("ES", "IT"), abs_threshold = 20, rel_threshold = 0.5)
#' }
#'
#' @export
nfsa_revision_T0801SA <- function(country,
  ...
  if (type == "version"){
    prev_db <-  nfsa_new_files <- list.files(path = "M:/nas/Rprod/data/q/new/sca/",
                                             recursive = FALSE,
                                             full.names = TRUE) |>
      as_tibble() |>
  ...
  if(length(unique(new_prev_db$ref_area)) == 1) {
    openxlsx::write.xlsx(new_prev_db,
                         file = paste0(output_sel,"/revisions_T0801SA_",unique(new_prev_db$ref_area),"_",as.character(format(Sys.time(), "%Y%m%d_%H%M%S")),".xlsx"),
                         overwrite = TRUE,
                         asTable = TRUE)

    cli::cli_alert_success(paste0("File created in ",output_sel,"/revisions_T0801_",unique(new_prev_db$ref_area),"_",as.character(format(Sys.time(), "%Y%m%d_%H%M%S")),".xlsx"))
  }
